...
def data_processing(*args):
	for index, file in enumerate(args):
		...
		logger.info("External data extracted successfully.")
		...
		logger = logging.getLogger(f"{name_u[0].upper()}")
		logger.info(f"{name_u[0].upper()} data successfully united.")
		...
		logger.info(f"{name_u[0].upper()} data successfully normalized.")
		
		# Save text file.
		df_university_data.to_csv(f"airflow/dags/files/{name_u[0]}_normalized.txt")
		logger.info(f"{name_u[0]}_normalized.txt successfully exported.")